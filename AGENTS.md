...
1. `docs/agents/harness-loop.md` — 端到端任务生命周期、门禁流转；含 RCA、**Spec-Driven 双阶段门禁**、**Git 功能分支门禁**、**开发并发须 `git worktree` 隔离**、**QC / QA 须在 feature 检出上下文（`Review cwd` / worktree）**、**内置 `@explore` 能力边界**（禁止承接方用 explore 代做交付）、可选前置门、与常见阶段化工作流的对照。
...
- **QC / QA 与 feature 检出**：QC 三审与 QA 验证针对 **已完成的 feature**；须在 PM 写明的 **`Review cwd / Worktree path`** 与 **`Working branch`** 上审查与跑测试/取证（通常即开发所用 worktree；**QA 与 QC 共用同一套字段**）。见 `harness-loop.md`「QC 三审、QA 验证与 feature 检出上下文」。
