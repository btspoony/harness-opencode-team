...
1. 定义变更假设（若涉及「库文档怎么查」或宿主差异，先对照 `library-docs-and-hosts.md`，避免与 Cursor/OpenCode 双轨说明重复或漂移）。
2. 运行一组固定的代表性任务提示词。
3. 对比变更前后的行为。
4. 记录发现并更新提示词/文档。
5. 重复直到收益趋于平稳。
...
- 行为准则合规：是否符合 `coding-behavior-guidelines.md`（显式假设、最小解法、手术式改动、`Step -> verify`）
