...
| 必用（并行拆分）      | `dispatching-parallel-agents`（**仅当** ≥2 条实现轨并行；叠用下项）；会话内**由 PM 顺序**多代理时用 `subagent-driven-development`（**勿**顶替并行轨；**勿**与默认 `Delegation: forbidden` 同条 Superpowers — 见上文「Delegation 与 Superpowers 清单一致」） |
